...
class KeluarbiayaService {
    ...
    try {
      final response = await dio.get(
        ...
        queryParameters: {
          ...
          "limit": limit,
          "user_id": ?userId,
          if (status != null && status.isNotEmpty) "status_keluarbiaya": status,
        },
        options: Options(
  ...
  Future<List<String>> getStatusKeluarbiayas() async {
    String? token = await getToken();
    final response = await dio.get(
      "/keluarbiayas/statuskeluarbiayas",
      options: Options(
        responseType: ResponseType.json,
        headers: {'Authorization': 'Bearer $token'},
      ),
  ...
  Future<void> addKeluarbiaya(AddKeluarbiayaRequest request) async {
    try {
      String? token = await getToken();
      await dio.post(
        "/keluarbiayas/store",
